  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      try {
        if (user) {
          setCurrentUser(user)
          // Ensure user profile document exists before subscribing
          try {
            ...
            if (!snap.exists()) {
              ...
              // Touch lastLogin if doc exists
              await updateDoc(userRef, { lastLogin: new Date(), updatedAt: new Date() })
            }
          } catch (profileInitError) {
            console.error('User profile init error:', profileInitError)
          }
